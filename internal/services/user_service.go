...
type UserService interface {
	...
	UpdateProfile(ctx context.Context, currentUserID, targetUserID, fullName, email string, position *string, isAdmin bool) (*domain.User, error)
...
func (s *userService) UpdateProfile(ctx context.Context, currentUserID, targetUserID, fullName, email string, position *string, isAdmin bool) (*domain.User, error) {
	if currentUserID != targetUserID && !isAdmin {
		return nil, domain.ErrAccessDenied
	}
	if err := s.users.UpdateProfile(ctx, targetUserID, fullName, email, position); err != nil {
		return nil, err
	}
