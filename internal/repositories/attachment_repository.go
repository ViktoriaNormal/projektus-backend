...
type AttachmentRepository interface {
	...
	ListByComment(ctx context.Context, commentID uuid.UUID) ([]domain.Attachment, error)
	CreateForComment(ctx context.Context, commentID, uploadedBy uuid.UUID, fileName, filePath, contentType string, fileSize int64) (*domain.Attachment, error)
...
func (r *attachmentRepository) ListByComment(ctx context.Context, commentID uuid.UUID) ([]domain.Attachment, error) {
	rows, err := r.q.ListCommentAttachments(ctx, uuid.NullUUID{UUID: commentID, Valid: true})
	if err != nil {
		return nil, errctx.Wrap(err, "ListCommentAttachments", "commentID", commentID)
	}
	result := make([]domain.Attachment, 0, len(rows))
	for _, row := range rows {
		a := domain.Attachment{
...
func (r *attachmentRepository) CreateForComment(ctx context.Context, commentID, uploadedBy uuid.UUID, fileName, filePath, contentType string, fileSize int64) (*domain.Attachment, error) {
	row, err := r.q.CreateAttachment(ctx, db.CreateAttachmentParams{
	...
	if err != nil {
		return nil, errctx.Wrap(err, "CreateCommentAttachment", "commentID", commentID, "fileName", fileName)
	}
	a := &domain.Attachment{
		ID:          row.ID,
		FileName:    row.FileName,
