...
SET username   = COALESCE(NULLIF(sqlc.arg(username)::text, ''), username),
    ...
    alternative_contact_channel = CASE WHEN sqlc.arg(set_alt_contact_channel)::boolean THEN sqlc.arg(alternative_contact_channel) ELSE alternative_contact_channel END,
    alternative_contact_info    = CASE WHEN sqlc.arg(set_alt_contact_info)::boolean THEN sqlc.arg(alternative_contact_info) ELSE alternative_contact_info END,
    updated_at = NOW()
